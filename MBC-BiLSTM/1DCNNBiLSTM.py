...
class model(nn.Module):
    def __init__(self, num_classes):
        ...
        self.conv1 = nn.Conv1d(3, 64, kernel_size=20, stride=1)  # padding也可以是tuple:(1,2)，#【输入信号的通道，卷积产生的通道(卷积核个数)，卷积核尺寸，卷积步长】，输入：【batch_size，信号维度，信号的最大长度】
        self.conv2 = nn.Conv1d(64,64, kernel_size=20, stride=1)  # padding也可以是tuple:(1,2)，
        self.conv3 = nn.Conv1d(64,64, kernel_size=20, stride=1)
...
print('f1:{}'.format(f1))
# np.save('./res/10cnnbiLSTMp.npy',prediction_y)
# np.save('./res/10cnnbiLSTMl.npy',test_y)
np.save('./res/10cnnbiLSTMloss.npy',losslist)
np.save('./res/10cnnbiLSTMacc.npy',acclist)
