...
class model(nn.Module):
    def __init__(self, num_classes):
        ...
        self.conv1 = nn.Conv1d(3, 64, kernel_size=20, stride=20)  # padding也可以是tuple:(1,2)，#【输入信号的通道，卷积产生的通道(卷积核个数)，卷积核尺寸，卷积步长】，输入：【batch_size，信号维度，信号的最大长度】
        self.conv2 = nn.Conv1d(64,64, kernel_size=5, stride=1)  # padding也可以是tuple:(1,2)，
        self.conv3 = nn.Conv1d(64,64, kernel_size=5, stride=1)
        # self.lstm1 = nn.LSTM(input_size=3, hidden_size=128, num_layers=1,batch_first=True, bidirectional=False)
        # self.lstm2 = nn.LSTM(input_size=128, hidden_size=128, num_layers=1,batch_first=True, bidirectional=False)
        self.fc = nn.Sequential(
            nn.Linear(in_features=64*15, out_features=num_classes),
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, x):
        ...
        # hidden_state = torch.randn(1 , batch_size, 128).to(device)  # [num_layers(=1) * num_directions(=2), batch_size, n_hidden]
        # cell_state = torch.randn(1, batch_size, 128).to(device)
        ...
        # print(out[:,-1,:].shape)
        # out = self.fc(output)
        out = self.fc(out)  #  (batch_size, num_layers, hidden_size)
        # print(out.shape)
...
# np.save('./res/LSTMp.npy',prediction_y)
# np.save('./res/LSTMl.npy',test_y)
np.save('./res/cnnloss.npy',losslist)
np.save('./res/cnnacc.npy',acclist)
