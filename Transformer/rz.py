...
class model(nn.Module):
    ...
    def forward(self, enc_inputs):
        ...
        ## 大小为[64,300,2]
        enc_outputs=enc_outputs.view(-1,3200)
        ## 大小为[64,600]
...
if __name__ == '__main__':
    ...
    i = 6
    ##读取数据
    X_train, X_test, y_train, y_test = np.load('./res/rz/train_x{}.npy'.format(i)), np.load('./res/rz/test_x{}.npy'.format(i)), np.load(
        './res/rz/train_y{}.npy'.format(i)), np.load('./res/rz/test_y{}.npy'.format(i))
    ...
    for epoch in range(100):
        ...
        print('epoch: {}, Train acc: {}'.format(epoch, trainnum * 1.0 / X_train.shape[0]))
        Model.eval()
        num = 0
        ...
        print('epoch: {}, Test acc: {}'.format(epoch, num * 1.0 / X_test.shape[0]))
    ...
    print('Test acc: {}'.format(num * 1.0 / X_test.shape[0]))
    # print(frr)
    # print(far)
    print('FRR:{}'.format(1 - (frr * 1.0 / (X_test.shape[0]/2))))

    print('FAR:{}'.format(1 - (far * 1.0 / (X_test.shape[0]/2))))
