...
class model(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_features=49152, out_features=5),
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, enc_inputs):
        ...
        ## 大小为[32,768,5]
        enc_outputs=enc_outputs.view(-1,49152)
        ## 大小为[32,3840]
...
if __name__ == '__main__':
    ...
    ##读取数据
    data = np.load('./res/sensor1data.npy')
    label = np.load('./res/sensor1lab.npy')
    ...
    for epoch in range(20):
        ...
        print('epoch: {}, acc: {}'.format(epoch, trainnum * 1.0 / 208))
        Model.eval()
        num = 0
        ...
        print('epoch: {}, acc: {}'.format(epoch, num * 1.0 / 52))
    ...
    for i, (t, l) in enumerate(test_loader):
        ...
        print(l)
        num += torch.sum(pred == l)
    print('Test acc: {}'.format(num * 1.0 / 52))
