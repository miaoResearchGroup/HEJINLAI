...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, 300 + 1, 64))  # 位置编码，获取一组正态分布的数据用于训练
    ...
    def forward(self, enc_inputs):
        ...
        enc_outputs = torch.cat((cls_tokens, enc_outputs), dim=1)  # 将分类令牌拼接到输入中，x的shape (b, n+1, 1024)
        enc_outputs += self.pos_embedding[:, :(n + 1)]
...
class model(nn.Module):
    def __init__(self,pool='cls'):
        ...
        self.mlp_head = nn.Sequential(
            nn.LayerNorm(d_model),  # 正则化
            nn.Linear(d_model, 2)  # 线性输出
        )
    ...
    def forward(self, enc_inputs):
        ...
        ## 经过线性层大小为[64,300,64]
        x, enc_self_attns = self.encoder(enc_outputs)
        ## 得到大小为[64,300,64]
        ...
        # ## 大小为[64,300,2]
        # enc_outputs=enc_outputs.view(-1,600)
        # ## 大小为[64,600]
...
if __name__ == '__main__':
    ...
    print('f1:{}'.format(f1))
    # np.save('./res/cnnbiLSTMp.npy',prediction_y)
    # np.save('./res/cnnbiLSTMl.npy',test_y)
    # np.save('./res/cnnbiLSTMloss.npy',losslist)
    # np.save('./res/cnnbiLSTMacc.npy',acclist)
